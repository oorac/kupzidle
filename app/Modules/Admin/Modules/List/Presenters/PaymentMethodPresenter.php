...
final class PaymentMethodPresenter extends BasePresenter
{
    /**
    ...
    protected function createComponentForm(): Form
    {
        ...
        $form = $builder->create($this->paymentMethod);
    ...
    {
        if (! $this->paymentMethod = $this->paymentMethodRepository->find($id)) {
    ...
    {
        $this->mapper->mapForm($this->paymentMethod, $form);
    ...
    protected function createComponentListGrid(string $name): Datagrid
    {
        ...
        $grid->addColumnText('title', $this->translator->translate('forms.title'))
            ->setRenderer(function (PaymentMethod $paymentMethod) {
        ...
        $grid->addColumnText('store', $this->translator->translate('forms.store'))
            ->setRenderer(function (PaymentMethod $paymentMethod) {
                return $paymentMethod->getStore()->getTitle();
            })
            ->setEditableValueCallback(
                function (PaymentMethod $paymentMethod) {
                    return $paymentMethod->getStore()->getId();
                }
            )
            ->setFilterText();
        ...
        $inlineEdit->onSetDefaults[] = static function (Container $container, PaymentMethod $paymentMethod): void {
            $container->setDefaults([
                'id' => $paymentMethod->getId(),
                'title' => $paymentMethod->getTitle(),
                'code' => $paymentMethod->getCode(),
                'store' => $paymentMethod->getStore()->getId()
            ]);
        };

        $inlineEdit->onSubmit[] = function ($id, $values): void {
            if (! $paymentMethod = $this->paymentMethodRepository->find((int) $id)) {
            ...
            $paymentMethod->setTitle($values->offsetGet('title'))
        ...
        $inlineAdd->onSubmit[] = function (ArrayHash $values): void {
            if ($this->paymentMethodRepository->findOneBy(['title' => $values->offsetGet('title')])) {
    ...
    {
        if (! $paymentMethod = $this->paymentMethodRepository->find($id)) {
            $this->flashMessage($this->translator->translate('errors.RowDoesntExist'), self::FM_ERROR);
            if ($this->isAjax()) {
                $this->redrawControl('flashes');
                return;
